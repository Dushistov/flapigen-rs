...
fn get_gcc_system_include_dirs() -> Result<Vec<PathBuf>, String> {
    ...
    try!(try!(gcc_process.stdin.ok_or(format!("can not get stdin of {}", GCC_CMD).as_str()))
        .write_all("\n".as_bytes()).map_err(|err| err.to_string()));

    let mut gcc_output = String::new();
    try!(try!(gcc_process.stderr.ok_or(format!("can not get stderr of {}", GCC_CMD).as_str()))
         .read_to_string(&mut gcc_output).map_err(|err| err.to_string()));

    const BEGIN_PAT: &'static str = "\n#include <...> search starts here:\n";
    const END_PAT: &'static str = "\nEnd of search list.\n";
    let start_includes = try!(gcc_output.find(BEGIN_PAT)
                              .ok_or(format!("No '{}' in output from {}", BEGIN_PAT, GCC_CMD).as_str())) + BEGIN_PAT.len();
    let end_includes = try!((&gcc_output[start_includes..]).find(END_PAT)
                            .ok_or(format!("No '{}' in output from {}", END_PAT, GCC_CMD).as_str())) + start_includes;
...
pub fn gen_binding<P>(target: &str, include_dirs: &[P], c_headers: &[&str], output_rust: &Path) -> Result<(), String>
    where P: AsRef<Path> {
    ...
    bindings = c_headers[1..].iter().fold(Ok(bindings), |acc: Result<bindgen::Builder, String>, header| {
        let c_file_path = search_file_in_directory(include_dirs, header)
                               .map_err(|_| format!("Can not find {}", header))?;
...
fn main() {
    ...
    gen_binding(&target, &include_dirs, &["jni.h", "android/bitmap.h"], &Path::new("src/android_c_headers.rs")).unwrap();
    ...
    env::set_var("RUST_SWIG_JNI_JAVA_PACKAGE", "net.akaame.myapplication");
    env::set_var("RUST_SWIG_JNI_JAVA_OUTPUT_DIR", "app/src/main/java/net/akaame/myapplication");
    registry.expand("rust_swig_test_jni", &src, &dst).unwrap();

    let cur_dir = env::current_dir().unwrap();
    let arm_libs_path = cur_dir.join("src/obj/local/armeabi");
    println!("cargo:rustc-link-search=native={}", arm_libs_path.to_str().unwrap());
