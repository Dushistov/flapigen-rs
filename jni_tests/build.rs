...
fn gen_binding(include_dirs: &[&Path],
               c_headers: &[&str],
               output_rust: &Path)
               -> Result<(), String> {
    ...
    let generated_bindings = try!(bindings
                                      .generate()
                                      .map_err(|_| "Failed to generate bindings".to_string()));
    try!(generated_bindings
...
fn main() {
    ...
    gen_binding(&[&java_include_dir, &java_sys_include_dir],
                &["jni.h"],
                &Path::new(&env::var("OUT_DIR").unwrap()).join("jni_c_header.rs"))
