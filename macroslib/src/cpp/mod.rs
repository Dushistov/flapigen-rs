...
"##,
    ...
    let template_class_name = format!("{}Template", class.name);
    write!(
        cpp_include_f,
        r#"// Automaticaly generated by rust_swig
#pragma once
...
namespace {namespace} {{
{doc_comments}
template<unsigned OwnershipType, typename SelfPtrType>
class {template_class_name} {{
public:
    using {class_name} = {template_class_name}<0, typename std::remove_const<SelfPtrType>::type>;
    {template_class_name}(const {template_class_name}&) = delete;
    {template_class_name} &operator=(const {template_class_name}&) = delete;
    {template_class_name}({template_class_name} &&o): self_(o.self_)
    {{
        o.self_ = nullptr;
    }}
    ...
    explicit {template_class_name}(SelfPtrType o): self_(o) {{}}
...
"#,
                ...
                write!(
                    cpp_include_f,
                    r#"
    {template_class_name}({args_with_types})
    {{
...
"#,
        ...
        write!(
            ...
            r#"
private:
   template<unsigned OT = OwnershipType>
   static void free_mem(typename std::enable_if<OT == 0, SelfPtrType>::type &p)
   {{
        if (p != nullptr) {{
            {c_destructor_name}(p);
            p = nullptr;
   ...
   template<unsigned OT = OwnershipType>
   static void free_mem(typename std::enable_if<OT == 1, SelfPtrType>::type &p)
   {{
       p = nullptr;
   }}
public:
...
using {class_name} = {template_class_name}<0, {c_class_type} *>;
using {class_name}Ref = {template_class_name}<1, const {c_class_type} *>;
}}//{namespace} {{
"#,
        c_class_type = c_class_type,
        namespace = namespace_name,
