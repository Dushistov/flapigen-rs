...
use crate::{
    ...
    typemap::{
        ...
        CType, CTypes, ForeignTypeInfo, NotMergedData, RustTypeIdx, FROM_VAR_TEMPLATE,
        TO_VAR_TEMPLATE,
    },
    types::{
...
May be you need to use `private constructor = empty;` syntax?",
    ...
    fn init(&self, conv_map: &mut TypeMap, code: &[SourceCode]) -> Result<()> {
        ...
        for (module_name, data) in not_merged_data {
...
"##
            ...
            for elem in &data {
                if let NotMergedData::ForeignCode(ref fcode) = elem {
                    //TODO: use fcode.cfg_option
                    c_header_f
                        .write_all(
                            fcode
                                .code
