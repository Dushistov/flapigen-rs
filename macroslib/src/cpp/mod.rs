...
public:
    ...
    {class_ref_name} as_ref() const noexcept;
"#,
        c_class_type = c_class_type,
...
"#,
                ...
                if self_variant.is_read_only() {
                    ...
                    cpp_class_ref_code.push_str(&cpp_code.replace(
                        &format!("const {} &", class.name),
                        &format!("const {} &", class_ref_name),
                    ));
                }
...
inline {class_ref_name} {class_name}::as_ref() const noexcept
{{
   return {class_ref_name}{{ self_ }};
}}
}}//{namespace} {{
"#,
        c_class_type = c_class_type,
